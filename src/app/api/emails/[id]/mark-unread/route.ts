...
export async function POST(
  ...
  try {
    ...
    await markEmailSeen(id, session.userId, false);
    const flags = (email.flags || []).filter((x) => x !== "\\Seen");
    ...
    return fail(error instanceof Error ? error.message : "Failed marking unread", 400);
  }
}
